...
function doPost(e){
  try {
    ...
    // Registration is the public entry point. It does not expose the private API secret.
    if (event === 'registration' && body.public === true) {
      return publicRegistration(d);
    }

    // All internal events remain protected by the private API secret.
    if (!API_SECRET || body.secret !== API_SECRET) {
      return out({ok:false,message:'Unauthorized'});
    }
...
function publicRegistration(d) {
  ...
  if (!name || !email || !phone) {
    return out({ok:false,message:'Name, email and phone are required.'});
  }
  if (!/^[^\\s@]+@[^\\s@]+\\.[^\\s@]+$/.test(email)) {
    return out({ok:false,message:'Please enter a valid email address.'});
  }
  if (String(d.website || '').trim() !== '') {
    return out({ok:false,message:'Registration rejected.'});
  }
  ...
  if (lastRow > 1) {
    ...
    if (emails.includes(email)) {
      return out({ok:false,message:'A student with this email is already registered.'});
    }
  }
  ...
  return out({ok:true,event:'registration',studentId:studentId,message:'Registration successful.'});
}
...
function doGet(){
  return out({ok:true,service:'Baba Nanak Academy Google Sheets API',status:'online'});
}
